							...
							<tbody>
								<tr>
	                            	<td width="96"><strong>Status</strong></td>
                                  	...
                                  	<td width="132"><strong>Dispute Type</strong></td>
                              		<td width="146">
                              			<select name="type" id="type">
                                    		<option value="-1">All</option>
											<option value="0" <?=( isset($_GET["type"]) && ( $_GET["type"] == 0) )?'selected="selected"':"";?>>Dispute</option>
											<option value="1" <?=( isset($_GET["type"]) && ( $_GET["type"] == 1) )?'selected="selected"':"";?>>Re-Dispute #1</option>
											<option value="2" <?=( isset($_GET["type"]) && ( $_GET["type"] == 2) )?'selected="selected"':"";?>>Re-Dispute #2</option>
										</select>
									</td>
	                	...
	                	?>	                	
	                        ...
	                        <tbody>
	                        		...
	                        		foreach( $disputeList as $row): 
	                        			...
	                        			switch( $row->dispute_type ){
	                                		case 0:
												...
												//else $page = 'response';
												$page = 'response';		
												$dispute_status = ($row->dispute_type_status == 0)? 'Pending':'Completed';
	                                		...
	                                		case 2:
	                                			...
	                                			$isAlmostDue = ( (count($xtemp) > 1) && isset($_GET["alert"]) && ($_GET["alert"] == 1) && ($row->redispute21_status == 1) && ($row->dispute_type_status == 0) )?'almostdue':'';
	                                			//$dispute_status = ($row->redispute22_status == 0)? 'Pending':'Completed';
	                                			
	                                			if( $row->redispute22_status == 0 ){
	                                				/*if( strtotime( date('Y-m-d')) > strtotime($row->estimate_due_date_four_day ) ){
	                                					$dispute_status = ( isset($_GET["alert"]) && ($_GET["alert"] == 1) )?'<span style="color:red">Overdue</span>':'<span style="color:red">Overdue</span>';	
	                                				}else{
	                        	...
	                        	<tr class="<?=$isAlmostDue;?>">
	                                ...
	                                <td><?php echo $this->disputes->getDisputeOwner( ($row->dispute_process+1) ); ?></td>
	                                <td><?
	                                
	                                	//echo ( $row->redispute22_status == 0 && $row->dispute_type_status == 0 )?$row->estimate_dute_date:'';
	                        			//echo $row->estimate_dute_date.' '.$this->disputes->checkDisputeOnwerStatus($row->dispute_type,$row->twodays, $row->threedays, $row->dispute_process+1);
											...
											if( $row->redispute22_status == 0 && $row->dispute_type_status == 0  )	
	                                			//echo $row->estimate_dute_date.' '.$this->disputes->checkDisputeOnwerStatus($row->dispute_type,$row->twodays, $row->threedays, $row->dispute_process+1);
	                                			//echo $row->estimate_dute_date.' '.$this->disputes->checkDisputeOnwerStatus($row->dispute_type,$row->twodays, $row->threedays, $row->dispute_process+1);
	                                			...
	                                			if( ($row->dispute_type == 2) && ($row->dispute_type_status == 2)  ){
	                                					                                				
	                                				/*if( strtotime( date('Y-m-d')) > strtotime($row->estimate_due_date_four_day ) ){
	                                					echo 'Over 3 days';
	                                				}else{	                                					
	                                					echo $row->estimate_due_date_four_day;	                                					
	                                				}*/
													

	                                				//echo $row->estimate_due_date_four_day.' '.$this->disputes->checkDisputeOnwerStatus($row->dispute_type,$row->twodays, $row->threedays, $row->dispute_process+1);	                                				
	                                				echo date('m/d/Y', strtotime('+3 day', $row->last_update)).' '.$this->disputes->checkDisputeOnwerStatus($row->dispute_type,$days2, $days3, $row->dispute_process+1);	                                				
        ...
        <script>
        	...
        	$("#clearbtn").click( function(){
            	window.location = "<?=base_url();?>client/list4/";
        	})
