...
center_id */
		...
		$user = array(			
			'DX_username'						=> $data->user_name,
			'DX_access_id'						=> $data->access_id,			
			'DX_firstname'						=> $data->user_firstname,
			'DX_lastname'						=> $data->user_lastname,
			'DX_emailaddress'					=> $data->user_email,
			'DX_ext'							=> $data->user_ext,
			'DX_center_id'						=> $data->center_id,		
			'DX_logged_in'					    => TRUE
	...
	*/
	...
	{
		// Load Models
		$this->ci->load->model('loginmod/users', 'users');
		//$this->ci->load->model('loginmod/user_temp', 'user_temp');
		// $this->ci->load->model('dx_auth/login_attempts', 'login_attempts');
		...
		if ( ! empty($login) AND ! empty($password))
		{
			...
			// Get user query
			if ($query = $this->ci->users->get_user_by_username($login) AND $query->num_rows() == 1)
			{
				// Get user record
				$row = $query->row();
			...
			/* 	if ($row->banned > 0)
				...
				// {					
					...
					{
						//echo "<script>alert('sulud')</script>";
						// Log in user 
			...
			/* elseif ($query = $this->ci->user_temp->get_user_by_username($login) AND $query->num_rows() == 1)
			{
				// Set error message
